...
public class HideSwitchFragmentNavigator extends Navigator<FragmentNavigator.Destination> {
    ...
    public HideSwitchFragmentNavigator(@NonNull Context context, @NonNull FragmentManager manager,
                                       int containerId) {
        mContext = context;
    ...
    public NavDestination navigate(@NonNull FragmentNavigator.Destination destination, @Nullable Bundle args,
        ...
        //不是每次都创建，而是在fragmentManager里面找不到才去创建
        ...
        //没有添加的话才去添加
        if (!frag.isAdded()) {
            //通过添加的方式以及控制显示和隐藏的方式来切换fragment
